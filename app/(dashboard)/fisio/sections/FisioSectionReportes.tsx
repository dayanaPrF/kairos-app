...
interface MetricaRaw {
  ...
  created_at: string
  sesion_entrenamiento: { fecha: string; id_paciente: string }
  ejercicio: { nombre_ejercicio: string; icono: string | null }
}
...
export function FisioSectionReportes() {
  ...
  const [periodo, setPeriodo]       = useState<'7d' | '30d' | '90d' | 'todo'>('todo')
  ...
  const cargar = async () => {
    ...
    const { data: relPacientes, error: e1 } = await supabase
      .from('paciente_fisioterapeuta')
      .select('id_paciente')
      .eq('id_fisioterapeuta', user.id)
      .is('deleted_at', null)

    if (e1 || !relPacientes?.length) { setLoading(false); return }
    ...
    if (e2 || !sesiones?.length) { setLoading(false); return }
    ...
    if (periodo !== 'todo') {
      ...
      metricasQuery = metricasQuery.gte('created_at', desde.toISOString())
    }

    const { data: metricas, error: e3 } = await metricasQuery
    if (e3 || !metricas?.length) { setLoading(false); return }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text)', margin: 0 }}>📊 Reportes</h2>
        ...
        <div style={{ display: 'flex', gap: '4px', background: 'var(--bg)', borderRadius: '10px', padding: '4px', border: '1px solid var(--border)' }}>
          {([['7d', '7 días'], ['30d', '30 días'], ['90d', '90 días'], ['todo', 'Todo']] as const).map(([val, label]) => (
            <button key={val} onClick={() => setPeriodo(val)} style={{ padding: '6px 12px', borderRadius: '7px', border: 'none', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', transition: 'all .15s', background: periodo === val ? 'var(--blue)' : 'transparent', color: periodo === val ? '#fff' : 'var(--text-light)' }}>
              {label}
            </button>
          ))}
        </div>
      ...
      {loading ? <LoadingState /> : !kpis ? <EmptyState /> : (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '12px' }}>
            <KPICard icon="🎯" label="Puntuación IA global" value={`${kpis.promedio_global}`} unit="/100" color={scoreColor(kpis.promedio_global)} tendencia={kpis.tendencia_global} grande />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <DestacadoCard label="🏆 Mejor ejercicio"     nombre={kpis.mejor_ejercicio} score={ejercicios[0]?.promedio}                        tipo="mejor" />
            <DestacadoCard label="⚠️ Ejercicio a mejorar" nombre={kpis.peor_ejercicio}  score={ejercicios[ejercicios.length - 1]?.promedio} tipo="peor" />
          </div>

          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px', flexWrap: 'wrap', gap: '8px' }}>
              <h3 style={{ fontSize: '1rem', fontWeight: 800, color: 'var(--text)', margin: 0 }}>Rendimiento por ejercicio</h3>
              <div style={{ display: 'flex', gap: '6px' }}>
                {([['todos', 'Todos'], ['mejores', '≥ 70'], ['peores', '< 50']] as const).map(([val, label]) => (
                  <button key={val} onClick={() => setFiltro(val)} style={{ padding: '5px 12px', borderRadius: '20px', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 700, fontFamily: 'inherit', border: '1.5px solid', borderColor: filtro === val ? 'var(--blue)' : 'var(--border)', background: filtro === val ? 'var(--blue-xlight)' : 'transparent', color: filtro === val ? 'var(--blue)' : 'var(--text-light)', transition: 'all .15s' }}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            {ejerciciosFiltrados.length === 0 ? (
              <div style={{ padding: '40px 0', textAlign: 'center', opacity: 0.4, fontSize: '0.85rem' }}>Sin ejercicios en este rango</div>
            ) : (
...
function KPICard({ icon, label, value, unit, color, tendencia, grande }: {
  ...
  tendencia?: 'sube' | 'baja' | 'estable'; grande?: boolean
}) {
  const paleta = {
  ...
  return (
    <div className="dash-card" style={{ padding: '18px 20px', borderLeft: `3px solid ${border}` }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', flexShrink: 0 }}>{icon}</div>
        {tendencia && <span style={{ fontSize: '1rem' }}>{tendencia === 'sube' ? '↗' : tendencia === 'baja' ? '↘' : '→'}</span>}
      </div>
      <div style={{ marginTop: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '3px' }}>
          <span style={{ fontSize: grande ? '2rem' : '1.6rem', fontWeight: 800, color: text, lineHeight: 1 }}>{value}</span>
          {unit && <span style={{ fontSize: '0.8rem', color: text, opacity: 0.7, fontWeight: 600 }}>{unit}</span>}
        </div>
        <div style={{ fontSize: '0.72rem', color: 'var(--text-light)', marginTop: '4px', fontWeight: 600 }}>{label}</div>
        {tendencia && (
          <div style={{ fontSize: '0.7rem', marginTop: '4px', fontWeight: 600, color: tendencia === 'sube' ? '#3b6d11' : tendencia === 'baja' ? '#a32d2d' : 'var(--text-light)' }}>
...
function DestacadoCard({ label, nombre, score, tipo }: { label: string; nombre: string; score?: number; tipo: 'mejor' | 'peor' }) {
  ...
  return (
    <div className="dash-card" style={{ padding: '16px 20px', borderTop: `3px solid ${border}` }}>
      <div style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--text-light)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>{label}</div>
      <div style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--text)', marginBottom: '6px' }}>{nombre}</div>
      {score !== undefined && (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: bg, color: text, padding: '4px 10px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 700 }}>
          {score}/100
        </div>
      )}
    </div>
...
function EjercicioRow({ ejercicio: e, posicion, total }: { ejercicio: EjercicioKPI; posicion: number; total: number }) {
  ...
  return (
    <div className="dash-card" style={{ padding: '16px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
        <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: posicion <= 3 ? bg : 'var(--bg)', color: posicion <= 3 ? text : 'var(--text-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '0.8rem', flexShrink: 0, border: `1px solid ${posicion <= 3 ? bar + '44' : 'var(--border)'}` }}>
          {posicion}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: '160px' }}>
          <span style={{ fontSize: '1.1rem' }}>{e.icono}</span>
          <span style={{ fontWeight: 700, fontSize: '0.88rem', color: 'var(--text)' }}>{e.nombre_ejercicio}</span>
        </div>
        <div style={{ flex: 1, minWidth: '80px' }}>
          <div style={{ height: '8px', borderRadius: '4px', background: 'var(--bg)', overflow: 'hidden', border: '1px solid var(--border)' }}>
            <div style={{ height: '100%', width: `${e.promedio}%`, background: bar, borderRadius: '4px', transition: 'width .4s ease' }} />
          </div>
        </div>
        <div style={{ minWidth: '56px', textAlign: 'right', fontWeight: 800, fontSize: '1rem', color: text }}>
          {e.promedio}<span style={{ fontSize: '0.7rem', fontWeight: 600, opacity: 0.7 }}>/100</span>
        </div>
        <div style={{ color: tendenciaColor, fontWeight: 700, fontSize: '0.9rem', minWidth: '20px', textAlign: 'center' }}>{tendenciaIcon}</div>
      ...
      {e.historico.length > 1 && <MiniSparkline historico={e.historico} color={bar} />}
    </div>
  )
}
...
function MiniSparkline({ historico, color }: { historico: { fecha: string; score: number }[]; color: string }) {
  ...
  const pts = historico.map((_, i) => {
    const x = pad + (i / (historico.length - 1)) * (W - pad * 2)
    const y = H - pad - ((historico[i].score - min) / rango) * (H - pad * 2)
    return `${x},${y}`
  })
  return (
    <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '10px' }}>
      <span style={{ fontSize: '0.68rem', color: 'var(--text-light)', fontWeight: 600, flexShrink: 0 }}>Evolución</span>
...
function LoadingState() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '12px' }}>
        {[1,2,3,4].map(i => <div key={i} className="dash-card" style={{ height: '100px', opacity: 0.3, background: 'var(--bg)' }} />)}
      </div>
      <div style={{ textAlign: 'center', opacity: 0.4, fontSize: '0.82rem', padding: '20px 0' }}>Calculando métricas IA...</div>
    </div>
  )
}
...
function EmptyState() {
  return (
    <div className="dash-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 20px', gap: '12px', opacity: 0.5, textAlign: 'center' }}>
      <span style={{ fontSize: '3rem' }}>📊</span>
      <div style={{ fontWeight: 800, fontSize: '1.1rem', color: 'var(--text)' }}>Sin métricas en este período</div>
